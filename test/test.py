...
#    im = cv2.imread(args["image"], 0)
    ...
    for im_scaled in pyramid_gaussian(im, downscale=downscale):
        ...
        for (x, y, im_window) in sliding_window(im_scaled, min_wdw_sz, step_size):
            ...
            # cv2.imshow("hog_image", im_window)
...
#                print "Scale ->  {} | Confidence Score {} \n".format(scale,clf.decision_function(fd))
            ...
            if visualize_det:
                ...
                for x1, y1, _, _, _  in cd:
                    ...
                    cv2.rectangle(clone, (x1, y1), (x1 + im_window.shape[1], y1 +
                        im_window.shape[0]), (0, 0, 0), thickness=1)
                cv2.rectangle(clone, (x, y), (x + im_window.shape[1], y +
                    im_window.shape[0]), (255, 255, 255), thickness=1)
                cv2.imshow("Sliding Window in Progress", clone)
                cv2.waitKey(30)
        # Move the the next scale
        scale+=1
        print scale, len(detections)
    ...
    for (x_tl, y_tl, _, w, h) in detections:
        # Draw the detections
        cv2.rectangle(im, (x_tl, y_tl), (x_tl+w, y_tl+h), (0, 255, 0), thickness=1)
    ...
    for (x_tl, y_tl, _, w, h) in detections:
        # Draw the detections
        cv2.rectangle(clone, (x_tl, y_tl), (x_tl+w,y_tl+h), (0, 255, 0), thickness=1)
